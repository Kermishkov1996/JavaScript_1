let renderer = {
    ...
    * @result {String} сгенерированный HTML-код таблицы(игрового поля).
    */
    generateBoard() {
        let board = '';
    ...
    /** 
    * Если ячейка с переданными координатами есть, то возвращается объект, а иначе null.
    * @param {{x: number, y:number}} position объект с координатами.
    * @returns {HTMLTableCellElement|null} объект ячейки если есть, в проивном случае null. 
    */
    getSquare(position) {
    ...
    /**
    * Метод рисует расположение пол-ля для указанной координаты.
